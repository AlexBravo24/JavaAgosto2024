...
public class Cajero implements IMetodos {
	...
	// Declarar una variable auxiliar para incrementar el folio en nuetros ticket
	private int folio = 1;
	...
	public Ticket retirar(int numeroCuenta, double monto) {
		...
		if (consultar(numeroCuenta) != null) {
			...
			if (monto > 9000) { // Validamos sis el monto excede el máximo del cajero
				...
				// Ahora si emitimos un nuevo ticket.
				ticket = new Ticket(folio++, new Date(), cuenta.getUsuario(), cuenta.getSaldo(), sucursal, 1);
				return ticket;

			}
		} else {
			System.out.println("No existe una asociada a ese cliente");
			return ticket;
		}
